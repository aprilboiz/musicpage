...
public interface SongRepository extends JpaRepository<Song, Long> {
    ...
    Optional<List<Song>> findSongsByAuthor(String author);
    Optional<Song> findSongByYoutubeID(String youtubeID);
    @Query(
            value = "SELECT DISTINCT s.id, s.title, s.author, s.youtube_id FROM songs s JOIN song_emotion e ON s.id = e.song_id WHERE e.emotion_id IN :emotionIDs",
            nativeQuery = true
    )
